...
const WatchHistory = () => {
      ...
      navigate('/series/waris', {
        state: {
          seriesData: {
            title: item.title,
            description:
              'A gripping drama series that follows the lives of a family entangled in power struggles, love, and betrayal.',
            genre: 'Drama, Family',
            year: '2024',
      ...
      navigate(`/movie/${item.title.toLowerCase().replace(/\s+/g, '-')}`, {
        state: {
          movieData: {
            title: item.title,
            description:
              item.title === 'Take It Easy Urvashi'
  ...
  return (
    <div
      className={`relative min-h-screen px-4 lg:px-20 py-16 font-sans overflow-hidden ${theme.background} ${theme.text}`}
    >
      {/* Blurred Background */}
      ...
      <div className="flex items-center gap-3 mb-7">
        ...
        <p className={`text-4xl font-bold tracking-tight ${theme.text}`}>{`Watch History`}</p>
      </div>
        ...
        <div className="flex flex-wrap gap-4">
          {historyItems.map((item, index) => (
            <motion.div
              ...
              className={`relative w-56 h-72 rounded-xl overflow-hidden ring-1 ring-white/10 shadow-lg hover:scale-105 transition-transform hover:shadow-xl group ${theme.card}`}
            >
              {/* Background Image */}
              <img
                src={item.thumbnail}
